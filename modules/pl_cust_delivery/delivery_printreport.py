...
__all__ = ["DeliveryDevisReport","DeliveryReport"]
...
class DeliveryDevisReport(Report):
    __name__ = "pl_cust_delivery.devisreport"
...
class DeliveryReport(Report):
    __name__ = "pl_cust_delivery.deliveryreport"
    ...
    def execute(cls, ids, data):
        ...
        if len(ids) == 1:
            (folder,) = Folders.browse(ids)
            aux[-1] = "BonLivraison-{}".format(folder.name).replace('/','')
        else :
            aux[-1] = "BonLivraison"

        cls.mark_delivery_as_print(ids)

        return tuple(aux)

    @classmethod
    def mark_delivery_as_print(cls, folders):
        Folders = Pool().get("pl_cust_plfolders.folders")
        fold = Folders.browse(folders)
        if fold:
            with Transaction().new_transaction():
                is_print = []
                for f in fold :
                    for d in f.delivery_lines:
                        if d.bl_print:
                            continue
                
                        is_print.append(d)
                    
                Pool().get('pl_cust_delivery.deliveryline').write(is_print, {'bl_print': True})
    ...
    def get_context(cls, records, headers, data):
        ...
        group_by_date = {}
        is_print=[]
        for d in context["folder"].delivery_lines:
            if d.bl_print:
                continue
            ddate = my_format_date(d.delivery_date)
            if not group_by_date.get(ddate,None) :
                group_by_date[ddate] = []
            
            group_by_date[ddate].append(
                (
                    d.name,
                    d.comment,
                    d.devisline_id.quantity,
                    d.quantity,
                )
            )
        
        context["bl_tab"] = list(group_by_date.items())
